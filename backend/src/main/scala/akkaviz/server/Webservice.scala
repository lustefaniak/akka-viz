...
object ApiMessages {
  ...
  def read(str: String): akkaviz.protocol.ApiClientMessage = {
    upickle.default.read[akkaviz.protocol.ApiClientMessage](str)
  }

  def write(msg: akkaviz.protocol.ApiServerMessage): String = {
...
class Webservice(implicit fm: Materializer, system: ActorSystem) extends Directives with SubscriptionSession {
  ...
  def internalToApi: Flow[BackendEvent, akkaviz.protocol.ApiServerMessage, Any] = Flow[BackendEvent].map {
  ...
  def eventSerialization: Flow[akkaviz.protocol.ApiServerMessage, String, Any] = Flow[akkaviz.protocol.ApiServerMessage].map {
